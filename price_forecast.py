...
if uploaded_file is not None:
    ...
    filter_values = st.sidebar.multiselect("选择筛选值", unique_values)
    ...
    if target and target != "" and features and algorithms:
        # 删除包含空值的行
        data_cleaned = filtered_data.dropna(subset=features + [target])

        X = data_cleaned[features]
        y = data_cleaned[target]

        # 保留原始特征值
        original_features = data_cleaned[features].copy()
        ...
        for i, algorithm in enumerate(algorithms):
            with cols[i]:
                if algorithm == "线性回归":
                    ...
                    y_train_lstm = y_train.values.reshape(-1, 1)  # 将 Pandas Series 转换为 NumPy 数组并调用 reshape
                    y_test_lstm = y_test.values.reshape(-1, 1)  # 将 Pandas Series 转换为 NumPy 数组并调用 reshape
                    ...
                    rmse = math.sqrt(mse)
                    std_dev = np.std(y_test_np[3:] - predictions.flatten())
                    st.write(f"MSE: {mse:.2f}")
                    st.write(f"RMSE: {rmse:.2f}")
                    st.write(f"预测结果的标准差: {std_dev:.2f}")
                    models["LSTM"] = model
                    continue
                ...
                # 将原始特征值添加到预测结果中
                results_with_original = original_features.loc[X_test.index].copy()
                results_with_original["实际值"] = y_test.values
                results_with_original["预测值"] = predictions
                st.dataframe(results_with_original, use_container_width=True)
